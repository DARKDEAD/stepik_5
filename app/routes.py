...
def init_user():
    session["auth"] = {"login": False, "user_name": "", "user_mail": ""}
...
def render_index():
    ...
    return render_template("main.html", categories=categories, dishes=Dish,)
...
def render_addtocart():
    ...
    if not request.form.get("id_dish") == "":
        ...
        s_cart.append(
            {"id_dish": id_dish, "amount": 1, "price": dish.price, "title": dish.title}
        )
        session["cart"] = s_cart
        # s_cart = session.get('cart', {})
...
def render_deldish():
    ...
    resp = {"id": id_dish, "url": render_template("base_list_dish.html")}
...
def render_cart():
    ...
    if request.method == "POST":
        ...
        if form.validate_on_submit():
            add_new_order(
                session.get("cart"),
                {"user_mail": form.mail.data, "address": form.address.data,},
                phone,
            )
            session["cart"] = []
            return render_template("ordered.html")
...
def render_account():
    orders = (
        Order.query.filter(Order.mail == session["auth"]["user_mail"])
